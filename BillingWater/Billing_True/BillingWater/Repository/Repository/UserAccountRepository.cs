   ...
   public class UserAccountRepository : BaseRepository<UserAccount, AppDb>, IUserAccounts
    {
        ...
        public List<UserAccountViewModel> getUserAccount(string Username, string Password)
        {
            var get = (from u in app.UserAccounts
                       ...
                       }).Select(x => new UserAccountViewModel { AccountType = x.AccountType, AccountPassword = x.AccountPassword, AccountUserName = x.AccountUserName, AccountId = x.AccountId, AccountName = x.AccountName, Id = x.Id , AccountEmail = x.AccountEmail }).ToList();
            return get;
        }
